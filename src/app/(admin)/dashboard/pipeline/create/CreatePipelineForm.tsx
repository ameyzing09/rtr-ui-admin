...
import { createPipelineAction } from '@/lib/actions/pipeline';
import {  convertFormToPayload, type CreatePipelineFormData, type StageFormData } from '@/domain/pipelines/schemas';
import { StageEditor } from './StageEditor';
import { toast } from '@/components/ui/ToastProvider';
import Tooltip from '@/components/ui/Tooltip';
import { StagePreviewStrip } from './StagePreviewStrip';

interface CreatePipelineFormProps {
  jobId?: string; // Context-aware: if creating from a job
}

/**
 * Create Pipeline Form Component
 * Comprehensive form for creating a new pipeline with dynamic stages
 */
export function CreatePipelineForm({ jobId }: CreatePipelineFormProps) {
  ...
  // Form state
  const [formData, setFormData] = useState<CreatePipelineFormData>({
    name: '',
    description: '',
    stages: [
  ...
  // Update form data
  const updateFormData = (updates: Partial<CreatePipelineFormData>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = convertFormToPayload(formData);
      ...
      if (result.success) {
        ...
        if (jobId) {
          // Context-aware: redirect to job's pipeline assign screen
          router.push(`/dashboard/jobs/${jobId}/pipeline`);
        } else {
          // Default: redirect to pipeline details page
          router.push(`/dashboard/pipeline/${result.data.id}`);
        }
      } else {
      ...
      <div className="border-b border-gray-200 bg-white px-6 py-4">
        ...
        <div className="mt-4">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Set up a new interview pipeline with custom stages
          </p>
        </div>
      </div>
