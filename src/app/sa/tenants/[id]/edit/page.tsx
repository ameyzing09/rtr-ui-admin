...
export default function EditTenantPage() {
  ...
  const {
    ...
    error: loadError,
    refetch
  } = useApiRequest<TenantDetail>(tenantId ? `/api/tenants/${tenantId}` : null);

  // Centralized mutation for updateTenant
  const {
    mutate: updateTenant,
    loading: isSaving,
    error: saveError
  } = useApiMutation<TenantDetail, UpdateTenantRequest>(tenantId ? `/api/tenants/${tenantId}` : '', 'PUT');
  ...
  if (loadError) {
    return <ErrorBanner error={loadError} onRetry={refetch} />;
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {saveError && saveError.code !== 'VALIDATION' && (
